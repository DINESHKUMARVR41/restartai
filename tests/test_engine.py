...
def test_live_full_stock_calls_only_first_supplier_and_builds_dashboard_plan():
    service = CalleService()
    service.mode = "live"
    engine = RecoveryEngine(service)
    r = request(idempotency_key="live-full", live_confirmed=True)
    called = []

    async def call_supplier(supplier, request, known_offers, idempotency_key=None):
        called.append(supplier.name)
        service.calls[idempotency_key] = {
            "call_id": idempotency_key,
            "supplier": supplier.name,
            "phone": supplier.phone,
            "phone_masked": service.mask_phone(supplier.phone),
            "status": "completed",
            "result": {"available_quantity": 20, "unit_price": 150, "currency": "INR", "delivery_hours": 2, "compatible": "yes", "can_fulfill": "yes"},
            "summary": "Confirmed 20 exact units.",
            "confidence": 1,
        }
        return {"supplier": supplier.name, "phone": supplier.phone, "quantity_available": 20, "unit_price": 150, "currency": "INR", "availability_hours": 2, "delivery_method": "delivery", "compatible": True, "confirmed": True, "compatibility_confidence": 1, "status": "FULL STOCK", "source": "CALL-E LIVE CALL", "call_id": idempotency_key, "notes": "Confirmed 20 exact units."}

    service.call_supplier_async = call_supplier
    d = asyncio.run(engine.start_async(r))
    assert called == ["Supplier A"]
    assert d["remaining_quantity"] == 0
    assert d["stage"] == "recovery_ready"
    assert d["recommended_plan"] is not None
    assert d["recommended_plan"]["legs"][0]["quantity"] == 20
    assert len(d["calls"]) == 1


def test_live_partial_stock_automatically_calls_next_supplier_until_shortfall_is_zero():
    service = CalleService()
    service.mode = "live"
    engine = RecoveryEngine(service)
    r = request(idempotency_key="live-partial", live_confirmed=True)
    called = []

    async def call_supplier(supplier, request, known_offers, idempotency_key=None):
        called.append((supplier.name, len(known_offers)))
        call_id = f"call_{supplier.name[-1]}"
        service.calls[call_id] = {"call_id": call_id, "supplier": supplier.name, "phone": supplier.phone, "phone_masked": service.mask_phone(supplier.phone), "status": "completed", "result": {"available_quantity": 8 if supplier.name == "Supplier A" else 12}}
        quantity = 8 if supplier.name == "Supplier A" else 12
        return {"supplier": supplier.name, "phone": supplier.phone, "quantity_available": quantity, "unit_price": 180 if supplier.name == "Supplier A" else 190, "currency": "INR", "availability_hours": 1, "delivery_method": "delivery", "compatible": True, "confirmed": True, "compatibility_confidence": 1, "status": "PARTIAL STOCK", "source": "CALL-E LIVE CALL", "call_id": call_id, "notes": f"Confirmed {quantity} exact units."}

    service.call_supplier_async = call_supplier
    d = asyncio.run(engine.start_async(r))
    assert [x[0] for x in called] == ["Supplier A", "Supplier B"]
    assert d["remaining_quantity"] == 0
    assert d["stage"] == "recovery_ready"
    assert sum(x["quantity"] for x in d["recommended_plan"]["legs"]) == 20
